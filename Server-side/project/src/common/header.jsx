...
import PredefinedRanges from '../src/PredefinedRanges'
...
import {
  ...
  import PropTypes from 'prop-types'
@withRouter
class OnlineFilter  extends React.Component {
  static propTypes = {
  ...
  constructor (props) {
    ...
    this.state = {
      filterObject:[],
      selectedfilterObject :   cookies.get('OnlineFilter', { path: '/' })
    }
  }
  ...
  _loadfilterObject =(input)=> {
    	if (!input) {
    		return Promise.resolve({ options: [] });
      }
      
      return axios.post('/OnlineFilter/predict',{
        JSONrequest:input.toString()
      })
      .then((response)=>{
        const filterObject=response.data
        console.log("the request filter filter is ()",filterObject)
        this.setState({ filterObject })
        return { options: filterObject }
  ...
  onChange =(selectedfilterObject)=>{
    
    /*
    * remove cookie if exist and set a new one
    * load cookie on render object 
    */
   ...
   //console.log('cookies',cookies)
    const cookies = new Cookies();
    cookies.remove('OnlineFilter', { path: '/' });
    cookies.set('OnlineFilter',selectedfilterObject, { path: '/' });
    console.log("the filter is" ,cookies.get('OnlineFilter')); 
    console.log( this.props.history)
    this.setState({ selectedfilterObject});
    this.props.router.push(this.props.router.location.pathname);

    //this.props.remote(selectedfilterObject);
   
  }
	render () {
  const { match, location, history } = this.props
  const {filterObject,selectedfilterObject} = this.state
  console.log('the online filter state',this.state);
  console.log('this.ptops',this.props)
        ...
        <VirtualizedSelect  
          ...
          multi={true}
          labelKey='predict'
          loadOptions={this._loadfilterObject}//
          optionHeight={73}
          ...
          value={selectedfilterObject}
          valueKey='key'
          placeholder= 'Filter Everything..'
          searchPromptText='Type to filter'
          maxHeight={400}
        />
		);
	}
    ...
    return (
      <Navbar.Header {...this.props}>
        <Navbar.Brand tabIndex='-1'>
          <a href='#'>
                    <img src='/imgs/common/logoOld.png' alt='SmartCart' width='78' height='40'  />
          </a>
        </Navbar.Brand>
      </Navbar.Header>
    );
...
const flagMenuItems = [
  ...
  { name: 'Hebrew', flag: 'israel', lang: 'he' }
          ...
          <Col xs={12}>
            <Navbar fixedTop fluid id='rubix-nav-header'>
              <Row>
                ...
                <Col xs={3} sm={4}>
                    <OnlineFilter  remote ={this.props.remote} />
                </Col>
                <Col xs={2} sm={3}>
                    <PredefinedRanges />
                </Col>
                <Col xs={4} sm={5} collapseRight className='text-right'>
                  <HeaderNavigation />
                </Col>
              </Row>
            </Navbar>
...
function FilterOptionRenderer ({ focusedOption, focusedOptionIndex, focusOption, key, labelKey, option, options, selectValue, style, valueArray, valueKey }) {
var i=512;
  return (
<div style={{margin:'0 0 0 15px'}}  >
    <InboxItem itemId={key}  key={i++}  unread src='/imgs/app/avatars/avatar5.png' selectValue ={selectValue} option ={option} imgClass={`border-${option.color}`} name={option.predict} labelValue={option.tableName} labelClass={`bg-Black fg-white`} description={<span>{option.label}</span>} date='Aug 20th' />
</div>
  )
}
    ...
    return (
      <a {...linkProps}>
        <div className='inbox-avatar'>
          ...
          <UserAvatar size="40" name={this.props.labelValue} />
